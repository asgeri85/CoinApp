package com.example.coinapp.base

import com.example.coinapp.model.ErrorResponse
    ...
    suspend fun <T> safeApiRequest(apiRequest: suspend () -> T): NetworkResult<T> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                when (throwable) {
                    is HttpException -> {
                        NetworkResult.Error(false,
                            errorBodyParse(throwable.response()?.errorBody()?.string()))
                    }
